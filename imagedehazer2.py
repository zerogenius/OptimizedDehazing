...
class ImageDehazer:
    ...
    def __AirlightEstimation(self, HazeImg):
        ...
        if len(HazeImg.shape) == 3:
            with ThreadPoolExecutor() as executor:
                self._A = list(executor.map(
                    lambda i: cv2.erode(HazeImg[:, :, i], kernel).max(), range(3)))
        else:
    ...
    def deHaze(self, HazeImg):
        scale_factor = 0.5  # Downsample by 50%
        haze_img = HazeImg
...
def process_image(input_path, output_path=None):
    ...
    cv2.imwrite("newfdehazing.jpg", dehazed_img)
    return output_path


process_image(r'C:\Users\Raymedis - IdeaPad\Downloads\handinverted.jpg')